 ...
 * Date: May 4th 2022 (May the Forth)
...
public class FrmMatchingGame extends JFrame {
    ...
    ImageIcon f = new ImageIcon("C:\\Users\\cfbur\\OneDrive\\Documentos\\ICS4U\\src\\Unit3\\Lesson14\\img\\subaru.jpg"); // good size
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrmMatchingGame frame = new FrmMatchingGame();
	...
	public FrmMatchingGame() {
		...
		btnPlay.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Code executed when btnPlay is clicked
				...
				// randomizer for cards using Math.random()
				for (int x=0; x<=11; x++) {
		...
		btnCard1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e1) {
				// string number for the cards to choose image
				String temp = cards.get(0);
				
				// if statements for selecting image using the randomized number
				if (temp.equals("0")) {
					btnCard1.setIcon(a);
				}
				...
				// makes it so the choosen image only gets displayed in 2 cards
				count ++;
				if (count == 1) {
        ...
        lblInstruction.setBounds(0, 400, 484, 26);
        contentPane.add(lblInstruction);
		
		JButton btnGuess = new JButton("Guess Again");
		btnGuess.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// code to be executed when btnGuess is clicked
				
				// if statements to check if the 2 cards selected are equal
				// if yes the images will change to a check mark to prove that they are correct
				// and the cardsleft counter will go down
				// if not then they will be flipped back
				count = 0;
				if (c1 == c2) {
					for (int y=1; y<=2; y++) {
						if(change[0] == 0) {
				...
				// if the card counter reachers 0
				// meaning you have gotten all the cards correctly 
				// a message will be displayed 
                if(cardsleft == 0){
                    lblInstruction.setBounds(200, 400, 484, 26);
